...
export default function CustomerAnalyticsPage({ customerId, onNavigate, onBackToSelector }) {
    const {
        customer,
        aiTasks,
        onboardingTasks,
    ...
    const handleAddAITask = async (taskData) => {
        try {
            const response = await analyticsService.addAIContentTask(customerId, taskData);
            if (response.success) {
    ...
    // ✅ YENİ: Onboarding Task ekleme
    const handleAddOnboardingTask = async (taskData) => {
        try {
            const response = await analyticsService.addOnboardingTask(customerId, taskData);
            if (response.success) {
                refresh(); // Listeyi yenile
            }
            return response;
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            {customer.companyName} - Süreç Takibi ve Yönetim
                        </p>
                    </div>
                ...
                <OnboardingProgressBoard
                    ...
                    onUpdateTask={updateOnboardingTask}
                    customerId={customerId}
                    onRefresh={refresh}
                />
